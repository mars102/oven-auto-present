  ...
  $('.userModal ').on('click', '.send-form button', function(e){
    ...
    if(error==0)
    {
      ...
      $.ajax({
       ...
       //$('.userModal .modal-title').html("");
       if(result==1){
         $('.userModal .modal-title').html("<b>Сообщение успешно доставлено!</b>");

          // $('.userModal .modal-title').html(result);
            $('.userModal .modal-body .send-form').html("");
           // $homepage = file_get_contents('./animationmesege.php', true);
           // $('.userModal .modal-body').append(homepage);
            $('.userModal').on('hidden.bs.modal', function (e) {
                ...
                if(formview==0) {
                  ...
                  // document.getElementById("smile").remove();
