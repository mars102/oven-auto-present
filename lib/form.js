...
$(document).ready(function(){
  ...
  $('.userModal ').on('click', '.send-form button', function(e){
    ...
    if(error==0)
    {
      var dataForm = $(".send-form").serialize();
      $.ajax({
        url: '/ajax/processor',
        type: 'post',
        data: dataForm+'&saleman=' + JSON.stringify($(".vigoda").html()),
        success: function(result) {
          var count_modal = 0;
          $('.userModal').modal('hide');
          $('.userModal').on('hidden.bs.modal', function (e) {
          
          if(count_modal>0) return;
             // функция resultMesege возвращает сообщения о успешной отправке при result =1 и о неуспешной в других случаях 
         resultMesege(result);
  ...
  $('.form-list ').on('click', 'button', function(e){
    ...
    if($(".check-client").hasClass("fa-circle-thin"))
    {
      $(".check-client").removeClass("fa-circle-thin");
      $(".check-client").addClass("fa-check-circle");
      $(".check-client").addClass("checked-client");
      $(".check-client").parent().find("#check-client").attr("checked","checked");
    }
    else{
      $(".check-client").addClass("fa-circle-thin");
      $(".check-client").removeClass("fa-check-circle");
      $(".check-client").removeClass("checked-client");
      $(".check-client").parent().find("#check-client").removeAttr("checked");
    }
