		...
		if(obj.attr("data-input")=="1")
		{
			...
			$(".company").each(function(i,item){
				...
				if(
					...
					//console.log($(this).find(".timer").attr("class"));
					$(this).find(".timer").addClass("nonTimer");
				}
			});
			//УДАЛЯЕМ ВСЁ ИЗ ЧЕКА
			...
			parent.css({"border-color":"#fc3"});
			parent.find("div").css("color","#333");
			parent.find(".timer").removeClass("nonTimer");
			...
			$(".company").each(function(i,item){
				...
				$(this).find(".timer").removeClass("nonTimer");
			});
		}
		...
		if(obj.attr("data-input")=="1")
		{
			...
			$(".smallcompany").each(function(i,item){
				...
				if(
					...
					$(this).find(".checkcompany").attr("data-input","0");
					//убираем курсор
					$(this).find(".checkcompany").css({"pointer-events":"none"});
					//очищаем содержимое, удаляем галку
					$(this).find(".checkcompany div").html("");
					//ставим бледную границу
					$(this).css({"border-color":"#efefef","border-width":"1px"});
					//цвет шрифта бледный
					$(this).find(".iphone4").css("color","#ccc");
					//цвет описания акции бледный
					//$(this).find(".description").css("color","#ccc");
					//ЦВЕТ ГРАНИЦЫ ЯЧЕЙКИ
					$(this).find(".checkcompany div").css({"border-color":"#efefef","pointer-events":"none"});
			...
			$(".smallcompany").each(function(i,item){
				...
				if(
					...
					$(this).find(".checkcompany").css({"pointer-events":"auto",});
					//СОДЕРЖИМОЕ ФА ИКОНКА
					$(this).find(".checkcompany div").html('<i class="fa fa-check" aria-hidden="true"></i>');
					//ВСТАВЛЯЕМ В ЧЕК
					$(".vigoda").append("<div class='addingcompany' id='"+$(this).find(".checkcompany").attr("data-id")+"'>"+$(this).find(".ofer").html()+"</div>");
					//ЧИСТИМ ЛИНИЮ
					$(".vigoda").append("<div class='clearfix'></div>");
			...
			$(".smallcompany").each(function(i,item){
				...
				$(this).find(".checkcompany").css({"border-color":"#ccc","pointer-events":"auto"});
				$(this).find(".iphone4").css("color","#333");
				$(this).find(".checkcompany div").css("border-color","#ccc");
			});
		}
...
function setTime(obj)
{
	...
	deadline = deadline+60*60*21*1000;
	...
	setInterval(function(){
		...
		if(hours==14 || hours==13 || hours==12 || hours==11) hour_str = '<b>'+hours+'</b><br/> часов';
		if(minutes==14 || minutes==13 || minutes==12 || minutes==11) min_str = '<b>'+minutes+'</b><br/> минут';
		if(seconds==14 || seconds==13 || seconds==12 || seconds==11) sec_str = '<b>'+seconds+'</b><br/> секунд';
...
var defHeight = $(".smallcompany").parent().height();
function openBlock(obj)
{
	var parent = obj.parent().parent().parent();
	var itemblock = parent.find(".company-text");
	//var itemHeight = parseInt(itemblock.css("height"));
	var itemHeight = itemblock.height();
	var parentHeight = parseInt(parent.css("height"));

	$(".smallcompany").each(function(){
		if($(this).find(".checkcompany").attr("data-input")!="1")
		{
		
			$(this).parent().find(".company-text").css("display","none");
			$(this).parent().css("height",defHeight+"px");
			//alert(" "+$(this).parent().height()-$(this).find(".checkcompany").attr("data-height"));
	...
	if(obj.attr("data-input")=="1")
	{
		...
		parent.css({"height":(itemHeight+parentHeight+20)+"px"});
		obj.attr("data-height",itemHeight);
	}
	else
	{
		parent.css({"height":(parentHeight-obj.attr("data-height")-20)+"px"});
...
$(document).ready(function(){
	...
	$(".smallcompany").on("click"," .checkcompany",function(){
		...
		if($(this).attr("data-input")=="0")
		{
			...
			$(".vigoda").append("<div class='clearfix'></div>");
			$(".addingcompany").find(".type_company").remove();
			...
			$(this).find("div").html("");
			parent.css({"border-color":"#ccc","border-width":"1px"});
	...
	$("#commer_offer").click(function()
	{
		var param = "";
		$(".addingcompany .checkcompany").each(function(i,item){
			if($(this).attr("data-input")==1) 
				param += $(this).attr("data-id")+"_";
		})
		document.location.href = $("#offer_download").attr("href")+"/"+param;
	})
