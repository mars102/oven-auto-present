...
jQuery(function($) {
  ...
  $(document).ready(function(){
      ...
      var fixedTachka = ($("#fixed-tachka").offset());
      ...
      $(window).scroll(function(){
          ...
          if($(this).scrollTop()>navtop.top)
          {
           ...
           if(dergatel2000==0&& $('*').is('.guider')){
            var verh=$(".guider").css("top");
            verh= Number(verh.slice(0, -2))-50;

            $(".guider").css("top",verh+"px");
            dergatel2000=1;
            }
          }
          ...
          if(typeof(fixedTachka) != "undefined" && fixedTachka !== null) {
            if($(this).scrollTop()>fixedTachka.top-50 && $(this).scrollTop()<fixedTachka.top+200) 
            { 
              //var right = Math.round($("#car_price").width()+$("#car_price").offset().left-$(".fixed-button").width())+"px";
              //console.log("offsetleft - "+$("#car_price").offset().left + "width - "+$("#car_price").width() + "sum - "+right);
              $("#fixed-tachka").addClass('fixed-tacka-price');
              $("#fixed-tachka").css("width",$(".container").width()+55);

            }
             else if($(this).scrollTop()>fixedTachka.top+400 || $(this).scrollTop()<fixedTachka.top)
            {
              $("#fixed-tachka").removeClass('fixed-tacka-price');
            //  $("#fixed-tachka").css("width","100%");
             // $("#fixed-tachka").css({"display":"none"});
            }
